...
static st_user_co_chargeing g_xj_co_charge[CHARGE_GUN_CNT_MAX];
...
static uint8_t    g_xj_phm_data[kXjPhmParamMax];  // 掉电存储内存区大小
...
const st_set_param_info set_501_param_list[] =
{
	...
	/*
	{
		&g_xj_params.safety_parameters.safety_36,
		type_uint32,
		NULL,
		NULL
	},
	{
		&g_xj_params.safety_parameters.safety_37,
		type_uint16x3,
		NULL,
		NULL
	},
	{
		&g_xj_params.safety_parameters.safety_38,
		type_uint16x3,
		NULL,
		NULL
	},
	{
		&g_xj_params.safety_parameters.safety_39,
		type_uint16x3,
		NULL,
		NULL
	},
	{
		&g_xj_params.safety_parameters.safety_40,
		type_uint64,
		NULL,
		NULL
	},
	{
		&g_xj_params.safety_parameters.safety_41,
		type_uint16,
		NULL,
		NULL
	},
	{
		&g_xj_params.safety_parameters.safety_42,
		type_uint16,
		NULL,
		NULL
	},
	{
		&g_xj_params.safety_parameters.safety_43,
		type_uint16,
		NULL,
		NULL
	},
	{
		&g_xj_params.safety_parameters.safety_44,
		type_uint16,
		NULL,
		NULL
	},
...
void sync_gun_status(xj_gun_status* p_xj_gun_status ,st_user_gun_info* p_user_gun_status ,uint16_t gun_cnt )
{
    ...
    if(NULL != p_xj_gun_status && NULL != p_user_gun_status && 0 != gun_cnt)
    {
        for(i = 0 ;i < gun_cnt ;i++)
        {
        	...
        	p_xj_gun_status[i].parallel_charging = p_user_gun_status[i].parallel_charging;
        	p_xj_gun_status[i].charge_power_kw = p_user_gun_status[i].charge_power_kw;
        	p_xj_gun_status[i].charging_temp = p_user_gun_status[i].chargeing_temp;
        }
	}
	else
	{
	...
	for(i = 0; i<_g_xj_params->gun_cnt ;i++)
	{
        ...
        if(TRUE != bill_info.info[i].start_bill_flag && (xj_work_stat_type_charging == g_xj_gun_status[i].work_stat || xj_work_stat_type_bist == g_xj_gun_status[i].work_stat))
        {
            ...
            for(j = 0;j < _g_xj_params->gun_cnt; j++){
            	if(g_xj_co_charge[j].charge_type == TRUE){
            		if(i == j){
            			memset((char*)&bill_info.info[g_xj_co_charge[j].minor_gun],00,sizeof(st_calculate_bill));
            			memset(&_g_xj_current_bill_info[g_xj_co_charge[j].minor_gun],0,sizeof(xj_bill_info));
            		}else if(i == g_xj_co_charge[j].minor_gun){
            			memset((char*)&bill_info.info[g_xj_co_charge[j].major_gun],00,sizeof(st_calculate_bill));
            			memset(&_g_xj_current_bill_info[g_xj_co_charge[j].major_gun],0,sizeof(xj_bill_info));
            		}
            	}
            }
			...
			for(j = 0;j < _g_xj_params->gun_cnt; j++){
				if(g_xj_co_charge[j].charge_type == TRUE){
					if(i == j){
						bill_info.info[g_xj_co_charge[j].minor_gun].last_kwh = g_xj_gun_status[g_xj_co_charge[j].minor_gun].current_meter_kwh_num;
						bill_info.info[i].last_kwh = g_xj_gun_status[i].current_meter_kwh_num;
						g_xj_gun_status[i].before_charge_meter_kwh_num =  g_xj_gun_status[i].current_meter_kwh_num;
						g_xj_gun_status[i].before_charge_meter_kwh_num += g_xj_gun_status[g_xj_co_charge[j].minor_gun].current_meter_kwh_num;
					}else if(i == g_xj_co_charge[j].minor_gun){
						bill_info.info[g_xj_co_charge[j].minor_gun].last_kwh = g_xj_gun_status[g_xj_co_charge[j].minor_gun].current_meter_kwh_num;
						bill_info.info[i].last_kwh = g_xj_gun_status[i].current_meter_kwh_num;
						g_xj_gun_status[i].before_charge_meter_kwh_num =  g_xj_gun_status[i].current_meter_kwh_num;
						g_xj_gun_status[i].before_charge_meter_kwh_num += g_xj_gun_status[g_xj_co_charge[j].major_gun].current_meter_kwh_num;
					}
				}
			}
			...
			for(j = 0;j < _g_xj_params->gun_cnt; j++){
				if(g_xj_co_charge[j].charge_type == TRUE){
					if(i == j){
						_g_xj_current_bill_info[i].start_charge_kwh_meter += g_xj_gun_status[g_xj_co_charge[i].minor_gun].current_meter_kwh_num;//_g_xj_current_bill_info[g_xj_co_charge[i].minor_gun].start_charge_kwh_meter;
					}else if(i == g_xj_co_charge[j].minor_gun){
						_g_xj_current_bill_info[i].start_charge_kwh_meter += g_xj_gun_status[g_xj_co_charge[i].major_gun].current_meter_kwh_num;//_g_xj_current_bill_info[g_xj_co_charge[i].major_gun].start_charge_kwh_meter;
					}
				}
			}
		...
		if(TRUE == bill_info.info[i].start_bill_flag)//更新账单
		{
			...
			if(bill_info.info[i].last_kwh > 0)
			{
				...
				for(j = 0;j < _g_xj_params->gun_cnt; j++){
					if(g_xj_co_charge[j].charge_type == TRUE){
						if(i == j){
							bill_info.info[g_xj_co_charge[j].minor_gun].accum_kwh = (uint32_t)(g_xj_gun_status[g_xj_co_charge[j].minor_gun].current_meter_kwh_num - bill_info.info[g_xj_co_charge[j].minor_gun].last_kwh);
							bill_info.info[i].accum_kwh += bill_info.info[g_xj_co_charge[j].minor_gun].accum_kwh;
						}else if(i == g_xj_co_charge[j].minor_gun){
							bill_info.info[g_xj_co_charge[j].major_gun].accum_kwh = (uint32_t)(g_xj_gun_status[g_xj_co_charge[j].major_gun].current_meter_kwh_num - bill_info.info[g_xj_co_charge[j].major_gun].last_kwh);
							bill_info.info[i].accum_kwh += bill_info.info[g_xj_co_charge[j].major_gun].accum_kwh;
						}
					}
				}
			}
			...
			for(j = 0;j < _g_xj_params->gun_cnt; j++){
				if(g_xj_co_charge[j].charge_type == TRUE){
					if(i == j){
						...
						bill_info.info[g_xj_co_charge[j].major_gun].last_kwh = g_xj_gun_status[g_xj_co_charge[j].major_gun].current_meter_kwh_num;
					}
				}
			}
			...
			if(bill_info.info[i].s_ctrl.init_section == 0){
				bill_info.info[i].s_ctrl.init_section = 1;
				bill_info.info[i].s_ctrl.index = bill_info.info[i].index;
			}
			if(bill_info.info[i].index != bill_info.info[i].s_ctrl.index){
				bill_info.info[i].s_ctrl.section_flag = 1;
				bill_info.info[i].s_ctrl.index = bill_info.info[i].index;
			}
			if(1 == bill_info.info[i].s_ctrl.section_flag){
				bill_info.info[i].charge_fee = 0;
				bill_info.info[i].service_fee = 0;
				bill_info.info[i].demurrage = 0;
				bill_info.info[i].s_ctrl.section_flag = 0;
			}else{
				_g_xj_current_bill_info[i].total_charge_fee -= bill_info.info[i].fee_section.charge_fee[bill_info.info[i].index];
				_g_xj_current_bill_info[i].total_service_fee -= bill_info.info[i].fee_section.service_fee[bill_info.info[i].index];
			}
			...
			bill_info.info[i].fee_section.charge_fee[bill_info.info[i].index] = bank_round(bill_info.info[i].charge_fee);
			bill_info.info[i].fee_section.service_fee[bill_info.info[i].index] = bank_round(bill_info.info[i].service_fee);
			...
			g_xj_gun_status[i].cumulative_charge_fee = _g_xj_current_bill_info[i].total_charge_fee + _g_xj_current_bill_info[i].total_service_fee;
/*************************************************end***********************************************/
			...
			for(j = 0;j < _g_xj_params->gun_cnt; j++){
				if(g_xj_co_charge[j].charge_type == TRUE){
					if(i == j){
						_g_xj_current_bill_info[g_xj_co_charge[j].minor_gun].end_charge_kwh_meter = g_xj_gun_status[g_xj_co_charge[j].minor_gun].current_meter_kwh_num;
						_g_xj_current_bill_info[i].end_charge_kwh_meter += _g_xj_current_bill_info[g_xj_co_charge[j].minor_gun].end_charge_kwh_meter;
					}else if(i == g_xj_co_charge[j].minor_gun){
						_g_xj_current_bill_info[g_xj_co_charge[j].major_gun].end_charge_kwh_meter = g_xj_gun_status[g_xj_co_charge[j].major_gun].current_meter_kwh_num;
						_g_xj_current_bill_info[i].end_charge_kwh_meter += _g_xj_current_bill_info[g_xj_co_charge[j].major_gun].end_charge_kwh_meter;
					}
				}
			}
			...
			if(xj_work_stat_type_charging != g_xj_gun_status[i].work_stat && xj_work_stat_type_bist != g_xj_gun_status[i].work_stat && xj_work_stat_type_stoping != g_xj_gun_status[i].work_stat)
			{
                ...
                if (g_xj_gun_status[i].start_flags == 1) {
                    ...
                    for(j = 0;j < _g_xj_params->gun_cnt; j++){
                    	if(g_xj_co_charge[j].charge_type == TRUE){
                    		if(i == j){
                    			memset(_g_xj_current_bill_info[g_xj_co_charge[j].minor_gun].kwh_amount,0,sizeof(_g_xj_current_bill_info[g_xj_co_charge[j].minor_gun].kwh_amount));
                    		}else if(i == g_xj_co_charge[j].minor_gun){
                    			memset(_g_xj_current_bill_info[g_xj_co_charge[j].major_gun].kwh_amount,0,sizeof(_g_xj_current_bill_info[g_xj_co_charge[j].major_gun].kwh_amount));
                    		}
                    	}
                    }
                ...
                for(j = 0;j < _g_xj_params->gun_cnt; j++){
                	if(g_xj_co_charge[j].charge_type == TRUE){
                		if(i == j){
                			memset((char*)&_g_xj_current_bill_info[g_xj_co_charge[j].minor_gun],0,sizeof(xj_bill_info));
                			memset((char*)&bill_info.info[g_xj_co_charge[j].minor_gun],00,sizeof(st_calculate_bill));
                			memset((char*)&g_xj_gun_status[g_xj_co_charge[j].minor_gun].soc_percent,00,(_OFFSETOF(xj_gun_status,user_tel) - _OFFSETOF(xj_gun_status,soc_percent)));
                		}else if(i == g_xj_co_charge[j].minor_gun){
                			memset((char*)&_g_xj_current_bill_info[g_xj_co_charge[j].major_gun],0,sizeof(xj_bill_info));
                			memset((char*)&bill_info.info[g_xj_co_charge[j].major_gun],00,sizeof(st_calculate_bill));
                			memset((char*)&g_xj_gun_status[g_xj_co_charge[j].major_gun].soc_percent,00,(_OFFSETOF(xj_gun_status,user_tel) - _OFFSETOF(xj_gun_status,soc_percent)));
                		}
                	}
                }
			}
...
static void xj_start_charge(const xj_start_charge_param* param)
{
    ...
    uint8_t charge_policy = 0;
    uint32_t charge_policy_param = 0;
    ...
    else
    {
        ...
        charge_policy = param->charge_policy;
        charge_policy_param = param->charge_policy_param;
        callback_start_charge(param->gun_index,err,(char*)param->charge_user_id,strlen(param->charge_user_id),param->user_tel,charge_policy,charge_policy_param);
...
static void xj_power_control(xj_power_ctrl_param *param)
{
	uint8_t gun_cnt = 0;
	uint8_t result = 0;
	xj_cmd_20 response;
	memset((char*)&response,0,sizeof(xj_cmd_20));
	gun_cnt = param->gun_index -1;
	if(gun_cnt >= _g_xj_params->gun_cnt)
	{
		result = 2;
		xj_pal_print_log(xj_log_warning,"gun_index wrong when power kwh! index=%d!\n",param->gun_index);
	}else if((xj_work_stat_type_bist != g_xj_gun_status[param->gun_index].work_stat) || (xj_work_stat_type_charging != g_xj_gun_status[param->gun_index].work_stat)){
		xj_pal_print_log(xj_log_warning,"charge stage wrong when power kwh! index=%d state=%d!\n",param->gun_index,g_xj_gun_status[param->gun_index].work_stat);
	}else{
		callback_power_control(gun_cnt,param->power_kwh);
	}
	memcpy(response.equipment_id,(uint8_t*)_g_xj_params->equipment_id,32);
	response.result = result;
	response.serial_code = param->serial_code;
	_xj_mqtt_send_resp_wrap(xj_cmd_type_20,(char*)&response,sizeof(xj_cmd_20),1);
}
...
/********************************************************
  * @Description：xj_code43 白名单设置
  * @Arguments	：
                param：code43 参数
  * @Returns	：
                无
 *******************************************************/
static void xj_code43_set_vin_write_list(xj_cmd_43* param)
{
	xj_cmd_42 revert;
	xj_cmd_43* cmd_43 = (xj_cmd_43*)param;

	memset((char*)&revert,00,sizeof(revert));
	revert.serial_code = cmd_43->serial_code;
	memcpy(revert.equipment_id,g_xj_params.equipment_id,XJ_MAX_EQUIPMENT_ID_LEN -1);
	//
	if(cmd_43->white_list_num > CMD_43_MAX_VIN_NUM){
		xj_pal_print_log(xj_log_warning,"code 42 is too long!\n");
		return ;
	}
	T_VIN_SET vin_list,vin_set_result;

//	callback_set_vin_white_list(&vin_list,&vin_set_result);

	_xj_mqtt_send_resp_wrap(xj_cmd_type_42,(char*)&revert,sizeof(xj_cmd_42),1);
}

/********************************************************
  * @Description：xj-code45 白名单清空
  * @Arguments	：
                param：code45 参数
  * @Returns	：
                无
 *******************************************************/
static void xj_code45_clean_vin_list(xj_cmd_45* param)
{
	xj_cmd_44 revert;
	xj_cmd_45* cmd_45 = (xj_cmd_45*)param;
	int8_t result = 1;
	memset((char*)&revert,00,sizeof(revert));
	revert.serial_code = cmd_45->serial_code;
	memcpy(revert.equipment_id,g_xj_params.equipment_id,XJ_MAX_EQUIPMENT_ID_LEN -1);
	//
//	callback_clean_vin_white_list(&result);

	_xj_mqtt_send_resp_wrap(xj_cmd_type_44,(char*)&revert,sizeof(xj_cmd_44),1);
}

static void xj_code37_set_card_write_list(xj_cmd_37* param)
{
	xj_cmd_38 revert;
	xj_cmd_37* cmd_37 = (xj_cmd_37*)param;

	memset((char*)&revert,00,sizeof(revert));
	revert.serial_code = cmd_37->serial_code;
	memcpy(revert.equipment_id,g_xj_params.equipment_id,XJ_MAX_EQUIPMENT_ID_LEN -1);
		//
	if(cmd_37->white_list_num > CMD_43_MAX_VIN_NUM){
		xj_pal_print_log(xj_log_warning,"code 37 is too long!\n");
		return ;
	}
	T_CARD_SET card_list,card_set_result;

//	callback_set_card_white_list(&card_list,&card_set_result);

	_xj_mqtt_send_resp_wrap(xj_cmd_type_38,(char*)&revert,sizeof(xj_cmd_38),1);
}

static void xj_code331_clean_card_write_list(xj_cmd_331* param)
{
	xj_cmd_332 revert;
	xj_cmd_331* cmd_331 = (xj_cmd_331*)param;
	int8_t result = 1;
	memset((char*)&revert,00,sizeof(revert));
	revert.serial_code = cmd_331->serial_code;
	memcpy(revert.equipment_id,g_xj_params.equipment_id,XJ_MAX_EQUIPMENT_ID_LEN -1);
	//
//	callback_clean_card_white_list(&result);

	_xj_mqtt_send_resp_wrap(xj_cmd_type_332,(char*)&revert,sizeof(xj_cmd_332),1);
}
...
void xj_helper_init_xj_params(xj_params* p,uint8_t gun_cnt)
{
	...
	if(!(p->safety_parameters.safety_12 > 0)){
		p->safety_parameters.safety_12 = 50;
	}
	if(!(p->safety_parameters.safety_13 > 0)){
		p->safety_parameters.safety_13 = 420;
	}
	if(!(p->safety_parameters.safety_14 > 0)){
		p->safety_parameters.safety_14 = 300;
	}
	if(!(p->safety_parameters.safety_15 > 0)){
		p->safety_parameters.safety_15 = 10;
	}
	if(!(p->safety_parameters.safety_16 > 0)){
		p->safety_parameters.safety_16 = 10;
	}
	if(!(p->safety_parameters.safety_17 > 0)){
		p->safety_parameters.safety_17 = 50;
	}
	if(!(p->safety_parameters.safety_18 > 0)){
		p->safety_parameters.safety_18 = 50;
	}
	if(!(p->safety_parameters.safety_19 > 0)){
		p->safety_parameters.safety_19 = 50;
	}
	if(!(p->safety_parameters.safety_20.param_1 > 0)){
		p->safety_parameters.safety_20.param_1 = 100;
	}
	if(!(p->safety_parameters.safety_20.param_2 > 0)){
		p->safety_parameters.safety_20.param_2 = 1;
	}
	if(!(p->safety_parameters.safety_21.param_1 > 0)){
		p->safety_parameters.safety_21.param_1 = 100;
	}
	if(!(p->safety_parameters.safety_21.param_2 > 0)){
		p->safety_parameters.safety_21.param_2 = 1;
	}
	if(!(p->safety_parameters.safety_22.param_1 > 0)){
		p->safety_parameters.safety_22.param_1 = 100;
	}
	if(!(p->safety_parameters.safety_22.param_2 > 0)){
		p->safety_parameters.safety_22.param_2 = 1;
	}
	if(!(p->safety_parameters.safety_23 > 0)){
		p->safety_parameters.safety_23 = 1;
	}
	if(!(p->safety_parameters.safety_24 > 0)){
		p->safety_parameters.safety_24 = 1;
	}
	if(!(p->safety_parameters.safety_25 > 0)){
		p->safety_parameters.safety_25 = 105;
	}
	if(!(p->safety_parameters.safety_26 > 0)){
		p->safety_parameters.safety_26 = 5;
	}
	if(!(p->safety_parameters.safety_27 > 0)){
		p->safety_parameters.safety_27 = 5;
	}
	if(!(p->safety_parameters.safety_28 > 0)){
		p->safety_parameters.safety_28 = 5;
	}
	if(!(p->safety_parameters.safety_29 > 0)){
		p->safety_parameters.safety_29 = 5;
	}
	if(!(p->safety_parameters.safety_30 > 0)){
		p->safety_parameters.safety_30 = 5;
	}
	if(!(p->safety_parameters.safety_31 > 0)){
		p->safety_parameters.safety_31 = 85;
	}
	if(!(p->safety_parameters.safety_32.param_1 > 0)){
		p->safety_parameters.safety_32.param_1 = 100;
	}
	if(!(p->safety_parameters.safety_32.param_2 > 0)){
		p->safety_parameters.safety_32.param_2 = 30;
	}
	if(!(p->safety_parameters.safety_33 > 0)){
		p->safety_parameters.safety_33 = 10;
	}
	if(!(p->safety_parameters.safety_34 > 0)){
		p->safety_parameters.safety_34 = 36;
	}
	if(!(p->safety_parameters.safety_35 > 0)){
		p->safety_parameters.safety_35 = 50;
	}
	/*
	if((p->safety_parameters.safety_36.param_1 < 5) || (p->safety_parameters.safety_36.param_1 > 10)){
		p->safety_parameters.safety_36.param_1 = 10;
	}
	if((p->safety_parameters.safety_36.param_2 <= 0) || (p->safety_parameters.safety_36.param_2 > 10)){
		p->safety_parameters.safety_36.param_2 = 10;
	}
	if((p->safety_parameters.safety_37.param_1 <= 0) || (p->safety_parameters.safety_37.param_1 > 100)){
		p->safety_parameters.safety_37.param_1 = 65;
	}
	if((p->safety_parameters.safety_37.param_2 <= 0) || (p->safety_parameters.safety_37.param_2 > 100)){
		p->safety_parameters.safety_37.param_2 = 65;
	}

	if((p->safety_parameters.safety_37.param_3 <= 0) || (p->safety_parameters.safety_37.param_3 > 10)){
		p->safety_parameters.safety_37.param_3 = 10;
	}
	if((p->safety_parameters.safety_38.param_1 <= 0) || (p->safety_parameters.safety_38.param_1 > 100)){
		p->safety_parameters.safety_38.param_1 = 30;
	}
	if((p->safety_parameters.safety_38.param_2 <= 0) || (p->safety_parameters.safety_38.param_2 > 100)){
		p->safety_parameters.safety_38.param_2 = 30;
	}
	if((p->safety_parameters.safety_38.param_3 <= 0) || (p->safety_parameters.safety_38.param_3 > 10)){
		p->safety_parameters.safety_38.param_3 = 10;
	}
	if((p->safety_parameters.safety_39.param_1 <= 0) || (p->safety_parameters.safety_39.param_1 > 50)){
		p->safety_parameters.safety_39.param_1 = 20;
	}
	if((p->safety_parameters.safety_39.param_2 <= 0) || (p->safety_parameters.safety_39.param_2 >50)){
		p->safety_parameters.safety_39.param_2 = 25;
	}
	if((p->safety_parameters.safety_39.param_3 <= 0) || (p->safety_parameters.safety_39.param_3 >10)){
		p->safety_parameters.safety_39.param_3 = 10;
	}
	if((p->safety_parameters.safety_40.param_1 <= 0) || (p->safety_parameters.safety_40.param_1 > 20)){
		p->safety_parameters.safety_40.param_1 = 11;
	}
	if((p->safety_parameters.safety_40.param_2 <= 0) || (p->safety_parameters.safety_40.param_2 > 20)){
		p->safety_parameters.safety_40.param_2 = 11;
	}
	if((p->safety_parameters.safety_40.param_3 <= 0) || (p->safety_parameters.safety_40.param_3 >20)){
		p->safety_parameters.safety_40.param_3 = 10;
	}
	if((p->safety_parameters.safety_40.param_4 <= 0) || (p->safety_parameters.safety_40.param_4 > 10)){
		p->safety_parameters.safety_40.param_4 = 10;
	}
	if((p->safety_parameters.safety_41 <= 0) || (p->safety_parameters.safety_41 > 5)){
		p->safety_parameters.safety_41 = 1;
	}
	if((p->safety_parameters.safety_42 > 60) || (p->safety_parameters.safety_42 < 10) ){
		p->safety_parameters.safety_42 = 15;
	}
	if((p->safety_parameters.safety_43 > 1000) || (p->safety_parameters.safety_43 < 10) ){
		p->safety_parameters.safety_43 = 300;
	}
	if((p->safety_parameters.safety_44 > 100) || (p->safety_parameters.safety_44 < 10) ){
		p->safety_parameters.safety_44 = 36;
	}
	*/
...
void _build_xj_cmd_104(xj_cmd_104* cmd_104,xj_gun_status* status){
	...
	if((g_xj_co_charge[status->gun_index].charge_type == TRUE) && (status->parallel_charging == TRUE)){
		cmd_104->before_charge_meter_kwh_num = g_xj_gun_status[status->gun_index].before_charge_meter_kwh_num;
		cmd_104->now_meter_kwh_num =  status->current_meter_kwh_num + g_xj_gun_status[g_xj_co_charge[status->gun_index].minor_gun].current_meter_kwh_num;
		if(xj_work_stat_type_charging == g_xj_gun_status[status->gun_index].work_stat){
			cmd_104->cum_charge_kwh_amount = status->current_meter_kwh_num + g_xj_gun_status[g_xj_co_charge[status->gun_index].minor_gun].current_meter_kwh_num- g_xj_gun_status[status->gun_index].before_charge_meter_kwh_num;
		}else{
			cmd_104->cum_charge_kwh_amount = 0;
		}
	}
	if((g_xj_co_charge[status->gun_index].charge_type == TRUE) && (g_xj_co_charge[status->gun_index].major_gun == status->gun_index)){
		cmd_104->before_charge_meter_kwh_num = g_xj_gun_status[status->gun_index].before_charge_meter_kwh_num;
		cmd_104->now_meter_kwh_num =  status->current_meter_kwh_num + g_xj_gun_status[g_xj_co_charge[status->gun_index].major_gun].current_meter_kwh_num;
		if(xj_work_stat_type_charging == g_xj_gun_status[status->gun_index].work_stat){
			cmd_104->cum_charge_kwh_amount = status->current_meter_kwh_num + g_xj_gun_status[g_xj_co_charge[status->gun_index].major_gun].current_meter_kwh_num- g_xj_gun_status[status->gun_index].before_charge_meter_kwh_num;
		}else{
			cmd_104->cum_charge_kwh_amount = 0;
		}
	}
	...
	*/
	...
	cmd_104->parallel_charging = status->parallel_charging;
	cmd_104->charge_power_kw = status->charge_power_kw;
	cmd_104->charging_temp = status->charging_temp;
}
...
void xj_incident_send_80(xj_cmd_80* send_buff)
{
    if(NULL != send_buff)
    {
        xj_cmd_81* cmd_81 = xj_mqtt_send_cmd(xj_cmd_type_80,send_buff,_g_xj_params->network_cmd_timeout,1);
    	if(NULL == cmd_81)
    	{
            xj_pal_print_log(xj_log_warning,"code 81 Failed to reply!\n");
    	}
    }
}
...
/********************************************************
  * @Description： xj参数514命令主动上报检查(连接后主动上报一次)
  * @Arguments	：null
  * @Returns	：null

 ********************************************************/
void _interval_work_for_cmd514_report(void)
{
    if (_g_cmd514_reported == 0)
    {
        send_514_all_notify();
        _g_cmd514_reported = 1;
    }
}
...
void _interval_work(void)
{
    ...
    if(1 == _g_is_login)
    {
        ...
        _interval_work_for_cmd514_report(); // 514参数主动上报
    }
    else
    {
        _g_cmd514_reported = 0;
    }
...
/********************************************************
  * @Description：触发 发送电表加密数据
  * @Arguments	： gun_cnt:触发的枪号 ，必须小于枪总数
			    0表示枪1 ,1表示枪2,以此类推
			    id: 订单号
			    meter_addr:电表地址
				encry_data:加密数据
...
int8_t xj_send_meter_encryp_data(uint8_t gun_cnt,char* id,char *meter_addr, char *encry_data)
{
	...
	if(gun_cnt < CHARGE_GUN_CNT)
	{
		data.gun_index = gun_cnt+1;
		memcpy(data.equipment_id,_g_xj_params->equipment_id,32);
		memcpy(data.charge_user_id,id,sizeof(data.charge_user_id));
		memcpy(data.meter_addr,meter_addr,sizeof(data.meter_addr));
		memcpy(data.encry_data,encry_data,sizeof(data.encry_data));
		result = xj_add_incident_handle(&xj_incident_info,xj_incident_type_send_80,(uint8_t*)&data,sizeof(data));
	}
	return result ;
}
...
int8_t xj_send_warning(uint8_t gun_cnt, uint8_t type,uint8_t *warning, uint32_t threshold)
{
    xj_cmd_120 cmd_120;
    int8_t result = -1;
...
/********************************************************
  ...
  * @Returns	：
                ...
                失败：-1
 *******************************************************/
int8_t xj_send_touch_warning_occured(uint8_t gun_cnt, uint8_t type,uint8_t* warning)
{
#if 0
    xj_cmd_120 cmd_120;
    int8_t result = -1;
    ...
    return xj_send_warning(gun_cnt,type,warning,th);
}

/********************************************************
  * @Description：并充模式发生与恢复时
  * @Arguments	：
			    major_gun:并充主枪 ，必须小于枪总数
			    0表示枪1 ,1表示枪2,以此类推
			    minor_gun:并充副枪 ，必须小于枪总数
			    0表示枪1 ,1表示枪2,以此类推
			    charge_type: 充电模式
			    0表示无并充 ,1表示并充
  * @Returns	：
...
{
	uint8_t i = 0;
	if(major_gun > _g_xj_params->gun_cnt || minor_gun > _g_xj_params->gun_cnt){
	     xj_pal_print_log(xj_log_Error,"Func:xj_send_co_changing  major_gun =%d,minor_gun =%d\n",major_gun,minor_gun);
	     return -1;
	}

	g_xj_co_charge[major_gun].charge_type = charge_type;
	g_xj_co_charge[major_gun].major_gun = major_gun;
	g_xj_co_charge[major_gun].minor_gun = minor_gun;

//	for(i = 0; i < _g_xj_params->gun_cnt;i++){
//		g_xj_co_charge[i].charge_type
//	}
	return 0;
}
...
void xj_send_Electric_meter_warning(uint8_t gun)
{
    ...
    {
        before_time = current_time;
        xj_send_touch_warning_occured(gun,type,XJ_SDK_ERR_300E);
    }
}
...
void _cmd_callback_start_charge(void* arg)
{
	...
	param.charge_policy = cmd_7->charge_policy;
	param.charge_policy_param = cmd_7->charge_policy_param;
...
void _cmd_callback_power_control(void* arg)
{
	xj_cmd_19* cmd_19 = (xj_cmd_19*)arg;
	xj_power_ctrl_param param;

	memset((char*)&param,00,sizeof(xj_power_ctrl_param));
	param.serial_code = cmd_19->serial_code;
	param.gun_index = cmd_19->gun_index;
	param.power_kwh = cmd_19->power_kwh;
	xj_add_incident_handle(&xj_incident_info,xj_incident_type_power_control,(uint8_t*)&param,sizeof(xj_power_ctrl_param));
}
...
//331
void _cmd_callback_card_clear(void* arg)
{
	xj_add_incident_handle(&xj_incident_info,xj_incident_type_clean_card_write_list,(uint8_t*)arg,sizeof(xj_cmd_331));
}
...
//37
void _cmd_callback_card_set(void *arg)
{
	xj_add_incident_handle(&xj_incident_info,xj_incident_type_set_card_write_list,(uint8_t*)arg,sizeof(xj_cmd_37));
}

//43
void _cmd_callback_vin_set(void *arg)
{
	xj_add_incident_handle(&xj_incident_info,xj_incident_type_set_vin_write_list,(uint8_t*)arg,sizeof(xj_cmd_43));
}

//45
void _cmd_callback_vin_clear(void* arg)
{
	xj_add_incident_handle(&xj_incident_info,xj_incident_type_clean_vin_write_list,(uint8_t*)arg,sizeof(xj_cmd_45));
}
...
void _cmd_callback_cmd_505(void* arg)
{
	 xj_l2_safety_prot_param 	param;
	 xj_cmd_506 revert;
	 xj_cmd_505* cmd_505 = (xj_cmd_505*)arg;
	 uint16_t p_data = 0;//当前指向
	 memset((char*)&revert,00,sizeof(revert));
	 memset((char*)&param,00,sizeof(param));
	 revert.serial_code = cmd_505->serial_code;

	 p_data += 4; //保留
	 memcpy(&param,&cmd_505->data[p_data],cmd_505->data_len-4);
	 if(cmd_505->data_len != (sizeof(param)+4)){
		 revert.set_result = 1;
	 }
	 else if(param.gun_index > _g_xj_params->gun_cnt)
	 {
		 revert.set_result = 5;
		 xj_pal_print_log(xj_log_warning,"gun_index wrong when 505! index=%d!\n",param.gun_index);
	 }else{
		 callback_safety_protect_param(&param,&revert.set_result);
	 }
	 revert.gun_index = param.gun_index;
	 memcpy(revert.charge_user_id,param.charge_user_id,XJ_MAX_EQUIPMENT_ID_LEN -1);
	 _xj_mqtt_send_resp_wrap(xj_cmd_type_506,(char*)&revert,sizeof(xj_cmd_506),1);
}
///---------------------------------507,508命令 start-----------------------------///

// 每个参数设置回调函数(<0:失败原因 0:与之前相同 >0:更新的长度)
typedef int (*item_set_callback)(void *data, uint16_t len);
// 每个参数获取回调函数(<0:失败原因 >=0 获取到的实际长度)
typedef int (*item_get_callback)(void *buffer, uint16_t size);
/// 参数项结构体
typedef struct{
	uint8_t             id;
	uint8_t             len;
    item_set_callback   set_callback;
    item_get_callback   get_callback;
} xj_cmd_507_item;

int param_507_item_set_1(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamHangZhouTspSwitch) != value)
    {
        xj_phm_write_byte(kXjPhmParamHangZhouTspSwitch, value);
        result = len;
    }
    return result;
}

int param_507_item_get_1(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamHangZhouTspSwitch);
    return 1;
}

int param_507_item_set_2(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamVirtualPowerShowSwitch) != value)
    {
        xj_phm_write_byte(kXjPhmParamVirtualPowerShowSwitch, value);
        result = len;
    }
    return result;
}

int param_507_item_get_2(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamVirtualPowerShowSwitch);
    return 1;
}

int param_507_item_set_3(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamVinSuppostSwitch) != value)
    {
        xj_phm_write_byte(kXjPhmParamVinSuppostSwitch, value);
        result = len;
    }
    return result;
}

int param_507_item_get_3(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamVinSuppostSwitch);
    return 1;
}

int param_507_item_set_4(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamCardReaderSuppostSwitch) != value)
    {
        xj_phm_write_byte(kXjPhmParamCardReaderSuppostSwitch, value);
        result = len;
    }
    return result;
}

int param_507_item_get_4(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamCardReaderSuppostSwitch);
    return 1;
}

int param_507_item_set_5(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamOutVolOverSwitch) != value)
    {
        xj_phm_write_byte(kXjPhmParamOutVolOverSwitch, value);
        result = len;
    }
    return result;
}

int param_507_item_get_5(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamOutVolOverSwitch);
    return 1;
}

int param_507_item_set_6(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamBsmFaultStopSwitch) != value)
    {
        xj_phm_write_byte(kXjPhmParamBsmFaultStopSwitch, value);
        result = len;
    }
    return result;
}

int param_507_item_get_6(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamBsmFaultStopSwitch);
    return 1;
}

int param_507_item_set_7(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamElectLockCheckSwitch) != value)
    {
        xj_phm_write_byte(kXjPhmParamElectLockCheckSwitch, value);
        result = len;
    }
    return result;
}

int param_507_item_get_7(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamElectLockCheckSwitch);
    return 1;
}

int param_507_item_set_8(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamEntranceGuardCheck) != value)
    {
        xj_phm_write_byte(kXjPhmParamEntranceGuardCheck, value);
        result = len;
    }
    return result;
}

int param_507_item_get_8(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamEntranceGuardCheck);
    return 1;
}

int param_507_item_set_9(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamAuxPowerSelect) != value)
    {
        xj_phm_write_byte(kXjPhmParamAuxPowerSelect, value);
        result = len;
    }
    return result;
}

int param_507_item_get_9(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamAuxPowerSelect);
    return 1;
}

int param_507_item_set_10(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamBhmTimeoutTime) != value)
    {
        xj_phm_write_byte(kXjPhmParamBhmTimeoutTime, value);
        result = len;
    }
    return result;
}

int param_507_item_get_10(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamBhmTimeoutTime);
    return 1;
}

int param_507_item_set_11(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamBrmTimeoutTime) != value)
    {
        xj_phm_write_byte(kXjPhmParamBrmTimeoutTime, value);
        result = len;
    }
    return result;
}

int param_507_item_get_11(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamBrmTimeoutTime);
    return 1;
}

int param_507_item_set_12(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamBcpTimeoutTime) != value)
    {
        xj_phm_write_byte(kXjPhmParamBcpTimeoutTime, value);
        result = len;
    }
    return result;
}

int param_507_item_get_12(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamBcpTimeoutTime);
    return 1;
}

int param_507_item_set_13(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamBroSet0x00Time) != value)
    {
        xj_phm_write_byte(kXjPhmParamBroSet0x00Time, value);
        result = len;
    }
    return result;
}

int param_507_item_get_13(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamBroSet0x00Time);
    return 1;
}

int param_507_item_set_14(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamBroSet0xAATime) != value)
    {
        xj_phm_write_byte(kXjPhmParamBroSet0xAATime, value);
        result = len;
    }
    return result;
}

int param_507_item_get_14(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamBroSet0xAATime);
    return 1;
}

int param_507_item_set_15(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamBclTimeoutTime) != value)
    {
        xj_phm_write_byte(kXjPhmParamBclTimeoutTime, value);
        result = len;
    }
    return result;
}

int param_507_item_get_15(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamBclTimeoutTime);
    return 1;
}

int param_507_item_set_16(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamBcsTimeoutTime) != value)
    {
        xj_phm_write_byte(kXjPhmParamBcsTimeoutTime, value);
        result = len;
    }
    return result;
}

int param_507_item_get_16(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamBcsTimeoutTime);
    return 1;
}

int param_507_item_set_17(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamBsmTimeoutTime) != value)
    {
        xj_phm_write_byte(kXjPhmParamBsmTimeoutTime, value);
        result = len;
    }
    return result;
}

int param_507_item_get_17(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamBsmTimeoutTime);
    return 1;
}

int param_507_item_set_18(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamOutVolOverValue) != value)
    {
        xj_phm_write_byte(kXjPhmParamOutVolOverValue, value);
        result = len;
    }
    return result;
}

int param_507_item_get_18(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamOutVolOverValue);
    return 1;
}

int param_507_item_set_19(void *data, uint16_t len)
{
    int result = 0;
    uint8_t value = *(uint8_t *)data;
    if (xj_phm_read_byte(kXjPhmParamStartChargeTimeout) != value)
    {
        xj_phm_write_byte(kXjPhmParamStartChargeTimeout, value);
        result = len;
    }
    return result;
}

int param_507_item_get_19(void *buffer, uint16_t size)
{
    *(uint8_t *)buffer = xj_phm_read_byte(kXjPhmParamStartChargeTimeout);
    return 1;
}

/// 参数表,id需要从小到大依次排列
xj_cmd_507_item g_cmd_507_param[] = 
{
    {1, 1, param_507_item_set_1, param_507_item_get_1},
    {2, 1, param_507_item_set_2, param_507_item_get_2},
    {3, 1, param_507_item_set_3, param_507_item_get_3},
    {4, 1, param_507_item_set_4, param_507_item_get_4},
    {5, 1, param_507_item_set_5, param_507_item_get_5},
    {6, 1, param_507_item_set_6, param_507_item_get_6},
    {7, 1, param_507_item_set_7, param_507_item_get_7},
    {8, 1, param_507_item_set_8, param_507_item_get_8},
    {9, 1, param_507_item_set_9, param_507_item_get_9},
    {10, 1, param_507_item_set_10, param_507_item_get_10},
    {11, 1, param_507_item_set_11, param_507_item_get_11},
    {12, 1, param_507_item_set_12, param_507_item_get_12},
    {13, 1, param_507_item_set_13, param_507_item_get_13},
    {14, 1, param_507_item_set_14, param_507_item_get_14},
    {15, 1, param_507_item_set_15, param_507_item_get_15},
    {16, 1, param_507_item_set_16, param_507_item_get_16},
    {17, 1, param_507_item_set_17, param_507_item_get_17},
    {18, 1, param_507_item_set_18, param_507_item_get_18},
    {19, 1, param_507_item_set_19, param_507_item_get_19},
};

/// 二分法查找返回具体项
xj_cmd_507_item* find_507_param_item(uint16_t id)
{
    xj_cmd_507_item *result = NULL;
    int min = 0;
    int max = sizeof(g_cmd_507_param) / sizeof(g_cmd_507_param[0]) - 1;
    int mid = 0;
    while(min <= max)
    {
        mid = (min+max)/2;
        if (g_cmd_507_param[mid].id > id)
        {
            max = mid-1;  //改变最大下标
        }
        else if(g_cmd_507_param[mid].id < id)
        {
            min = mid+1;  //改变最小下标
        }
        else
        {
            result = &g_cmd_507_param[mid];
            break;
        }
    }
    return result;
...
void _cmd_callback_cmd_507(void* arg)
{
    uint8_t msg_fault = 0;
    uint16_t id;
    uint16_t len;
    ...
    xj_cmd_507* cmd_507 = (xj_cmd_507*)arg;
    xj_cmd_508  cmd_508;
    cmd_508.success_cnt = 0;
    cmd_508.fail_cnt = 0;
    int cmd_508_len = sizeof(xj_cmd_508) - sizeof(cmd_508.fail_item);

    xj_cmd_507_head *head = (xj_cmd_507_head *)&cmd_507->data[0];
    xj_cmd_507_body *body = (xj_cmd_507_body *)(head + 1);
    uint16_t now_size = sizeof(xj_cmd_507_head);
    memcpy(&cmd_508.equipment_id[0], &head->equipment_id[0], sizeof(head->equipment_id));  

    // 校验长度是否正确
    for (i = 0; i < head->param_cnt; i++)
    {
        if (now_size + sizeof(xj_cmd_507_body) <= cmd_507->data_len)
        {
            now_size += sizeof(xj_cmd_507_body);
            /// 获取具体id项匹配数据长度
            xj_cmd_507_item *item = find_507_param_item(body->param_id);

            if (item != NULL && body->data_len == item->len && now_size + body->data_len <= cmd_507->data_len)
            {
                body = (xj_cmd_507_body *)((char *)body + sizeof(xj_cmd_507_body) + body->data_len);
                now_size += body->data_len;
            }
            else 
            {
                msg_fault = 1;
    ...
    if (!msg_fault)
    {
        ...
        for (i = 0; i < head->param_cnt; i++)
        {
            ...
            if (item != NULL)
            {
                int set_resut = item->set_callback(body + 1, body->data_len);
                if (set_resut >= 0)
                {
                    cmd_508.success_cnt++;
                    // 有更新，需要同步到文件
                ...
                {
                    cmd_508.fail_item[cmd_508.fail_cnt].id = body->param_id;
                    cmd_508.fail_item[cmd_508.fail_cnt].reason = 0 - set_resut;
                    cmd_508.fail_cnt++;
                    /// 超过最大长度
                    if (cmd_508.fail_cnt > CMD508_MAX_DATA_LEN)
                    {
                        //Todo
                        break;
                    }
            ...
            {
                cmd_508.fail_item[cmd_508.fail_cnt].id = body->param_id;
                cmd_508.fail_item[cmd_508.fail_cnt].reason = 1;
                cmd_508.fail_cnt++;
                /// 超过最大长度
                if (cmd_508.fail_cnt > CMD508_MAX_DATA_LEN)
                {
        ...
        cmd_508_len += sizeof(xj_cmd_508_fail_item) * cmd_508.fail_cnt;

        _xj_mqtt_send_resp_wrap(xj_cmd_type_508,(char*)&cmd_508, cmd_508_len, 1);
...
///---------------------------------511,512命令 start-----------------------------///
// 每个控制项回调函数(<0:失败原因 0: 成功)
typedef int (*item_ctrl_callback)(uint16_t param1, uint16_t param2);
/// 控制项结构体
typedef struct{
	uint8_t              id;
    item_ctrl_callback   ctrl_callback;
} xj_cmd_511_item;

int param_511_item_ctrl_1(uint16_t param1, uint16_t param2)
{
    return 0;
}

int param_511_item_ctrl_2(uint16_t param1, uint16_t param2)
{
    return 0;
}

int param_511_item_ctrl_3(uint16_t param1, uint16_t param2)
{
    return 0;
}

/// 控制表, id需要从小到大依次排列
xj_cmd_511_item g_cmd_511_param[] = 
{
    {1, param_511_item_ctrl_1},
    {2, param_511_item_ctrl_2},
    {3, param_511_item_ctrl_3},
};


/// 二分法查找返回具体项
xj_cmd_511_item* find_511_param_item(uint16_t id)
{
    xj_cmd_511_item *result = NULL;
    int min = 0;
    int max = sizeof(g_cmd_511_param) / sizeof(g_cmd_511_param[0]) - 1;
    int mid = 0;
    while(min <= max)
    {
        mid = (min+max)/2;
        if (g_cmd_511_param[mid].id > id)
        {
            max = mid-1;  //改变最大下标
        }
        else if(g_cmd_511_param[mid].id < id)
        {
            min = mid+1;  //改变最小下标
        }
        else
        {
            result = &g_cmd_511_param[mid];
            break;
        }
    }
    return result;
...
void _cmd_callback_cmd_511(void* arg)
{
    xj_cmd_511* cmd_511 = (xj_cmd_511*)arg;
    xj_cmd_512  cmd_512;
    memset(&cmd_512, 0, sizeof(cmd_512));
    if (cmd_511->data_len >= sizeof(xj_cmd_511_head))
    {
        xj_cmd_511_head *head = (xj_cmd_511_head *)&cmd_511->data[0];
        memcpy(&cmd_512.equipment_id[0], &head->equipment_id[0], sizeof(head->equipment_id));

        xj_cmd_511_item *item = find_511_param_item(head->param_id);
        if (item != NULL)
        {
            int ctl_result = item->ctrl_callback(head->param_value1, head->param_value2);
            if (ctl_result == 0)
            {
                cmd_512.result = 0;
                cmd_512.fail_reson = 0;
            }
            else
            {
                cmd_512.result = 1;
                cmd_512.fail_reson = 0 - ctl_result;
            }
        }
        else
        {
            cmd_512.result = 1;
            cmd_512.fail_reson = 1;
        }
        _xj_mqtt_send_resp_wrap(xj_cmd_type_512,(char*)&cmd_512, sizeof(cmd_512), 1);
    }
}
///---------------------------------511,512命令 end-----------------------------///

///---------------------------------513,514命令 start-----------------------------///
void _cmd_callback_cmd_513(void* arg)
{
    int i;
    xj_cmd_513* cmd_513 = (xj_cmd_513*)arg;
    xj_cmd_514  cmd_514;
    memset(&cmd_514, 0, sizeof(cmd_514));
    uint32_t    cmd_514_len = sizeof(xj_cmd_514) - sizeof(cmd_514.data);

    xj_cmd_513_head *head = (xj_cmd_513_head *)&cmd_513->data[0];
    xj_cmd_513_body *body = (xj_cmd_513_body *)(head + 1);

    memcpy(&cmd_514.equipment_id[0], &head->equipment_id[0], sizeof(head->equipment_id));
    
    // 0为请求所有参数
    if (head->req_cnt == 0)
    {
        uint8_t *p = (uint8_t *)&cmd_514.data[0];
        cmd_514.resp_cnt = sizeof(g_cmd_507_param) / sizeof(g_cmd_507_param[0]);
        for (i = 0; i < cmd_514.resp_cnt; i++)
        {
            *p++ = g_cmd_507_param[i].id;
            *p++ = 0;
            *p++ = g_cmd_507_param[i].len;
            g_cmd_507_param[i].get_callback(p, g_cmd_507_param[i].len);
            p += g_cmd_507_param[i].len;
        }
        cmd_514_len += (p - (uint8_t *)&cmd_514.data[0]);
    }
    else
    {
        uint8_t *p = (uint8_t *)&cmd_514.data[0];
        cmd_514.resp_cnt = head->req_cnt;
        for (i = 0; i < head->req_cnt; i++)
        {
             xj_cmd_507_item *item = find_507_param_item(body[i].id);
            /// 根据设置结果更新应答值
            if (item != NULL)
            {
                *p++ = item->id;
    ...
    _xj_mqtt_send_resp_wrap(xj_cmd_type_514,(char*)&cmd_514, cmd_514_len, 1);

}

// 发送514全部参数消息,主动上报
void send_514_all_notify(void)
{
    xj_cmd_514  cmd_514;
    uint32_t    cmd_514_len;
    int i;
    uint8_t *p;
    memset(&cmd_514, 0, sizeof(cmd_514));
    cmd_514_len = sizeof(xj_cmd_514) - sizeof(cmd_514.data);

    memcpy(cmd_514.equipment_id,_g_xj_params->equipment_id,sizeof(cmd_514.equipment_id));
    p = (uint8_t *)&cmd_514.data[0];
    cmd_514.resp_cnt = sizeof(g_cmd_507_param) / sizeof(g_cmd_507_param[0]);
...
void* _on_cmd_received_callback(void* cmd,xj_mqtt_cmd_enum cmd_type)
{
	...
	switch(cmd_type)
	{
		...
		case xj_cmd_type_11:
			...
			break;
		case xj_cmd_type_19:
			_cmd_callback_power_control(cmd);
			break;
	    case xj_cmd_type_23:
        	_cmd_callback_lock_control_order(cmd);
			break;
	    case xj_cmd_type_37:
	    	_cmd_callback_card_set(cmd);
	    	break;
	    case xj_cmd_type_43:
	    	_cmd_callback_vin_set(cmd);
	    	break;
	    case xj_cmd_type_45:
	    	_cmd_callback_vin_clear(cmd);
	    	break;
		...
		case xj_cmd_type_205:
        	...
        	break;
		case xj_cmd_type_331:
			_cmd_callback_card_clear(cmd);
			break;
		...
		case xj_cmd_type_503:
			...
			break;
		case xj_cmd_type_505:
			_cmd_callback_cmd_505(cmd);
			break;
		case xj_cmd_type_507:
			_cmd_callback_cmd_507(cmd);
			break;
		case xj_cmd_type_511:
			_cmd_callback_cmd_511(cmd);
			break;
		case xj_cmd_type_513:
			_cmd_callback_cmd_513(cmd);
			break;
...
static void System_param_init(st_user_gun_info* gun_status, st_user_bms_info* bms_status,uint8_t gun_cnt)
{
    ...
    xj_phm_init(&g_xj_phm_data[0], sizeof(g_xj_phm_data));  // 掉电参数初始化
}
...
void xj_incident_info_handle(st_xj_incident_info* info)
{
    ...
    if(info->incident_sum > 0)
    {
        ...
        switch(info->incident_list[info->incident_new].incident_name)
        {
            ...
            case xj_incident_type_power_control:
            {
            	xj_power_control((xj_power_ctrl_param*)param);
            	break;
            }
            ...
            case xj_incident_type_set_vin_write_list:
            {
            	xj_code43_set_vin_write_list((xj_cmd_43*)param);
            	break;
            }
            case xj_incident_type_clean_vin_write_list:
            {
            	xj_code45_clean_vin_list((xj_cmd_45*)param);
            	break;
            }
            case xj_incident_type_send_80:
            {
            	xj_incident_send_80((xj_cmd_80*)param);
            	break;
            }
            case xj_incident_type_set_card_write_list:
            {
            	xj_code37_set_card_write_list((xj_cmd_37*)param);
            	break;
            }
            case xj_incident_type_clean_card_write_list:
            {
            	xj_code331_clean_card_write_list((xj_cmd_331*)param);
            	break;
            }
