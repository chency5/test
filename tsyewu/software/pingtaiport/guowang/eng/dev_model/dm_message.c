...
int dm_msg_request(dm_msg_dest_type_t type, _IN_ dm_msg_request_t *request)
{
    ...
    payload_len = strlen(DM_MSG_REQUEST) + 10 + strlen(DM_MSG_VERSION) + request->params_len + strlen(request->method) + 1;
                        ...
                        char *rrpcid, int rrpcid_len, dm_msg_request_payload_t *request)
{
    ...
    message_len = strlen(DM_MSG_EVENT_RRPC_REQUEST_FMT) + request->id.value_length + DM_UTILS_UINT32_STRLEN + serviceid_len + rrpcid_len +
                                  ...
                                  _IN_ dm_msg_response_payload_t *response)
{
    ...
    message_len = strlen(DM_MSG_EVENT_SPECIFIC_POST_REPLY_FMT) + DM_UTILS_UINT32_STRLEN * 3 + strlen(identifier) + response->message.value_length + 1;
    message = DM_malloc(message_len);
    if (message == NULL)
    {
        return STATE_SYS_DEPEND_MALLOC;
...
int dm_msg_dev_core_service_dev(char **payload, int *payload_len)
{
    ...
    *payload_len = strlen(DM_MSG_DEV_CORE_SERVICE_DEV) + strlen(ip_addr) + DM_UTILS_UINT16_STRLEN + strlen(device_array) + 1;
    *payload = DM_malloc(*payload_len);
    if (*payload == NULL)
    {
                                        ...
                                        _IN_ char device_name[IOTX_DEVICE_NAME_LEN + 1], _OU_ dm_msg_request_t *request)
{
    ...
    /* Sign */
    sign_source_len = strlen(DM_MSG_THING_PROXY_PRODUCT_REGISTER_SIGN_SOURCE) + strlen(device_name) + strlen(product_key) + strlen(random) + 1;
...
static int _dm_msg_set_number(dm_msg_set_type_t type, int devid, char *key, lite_cjson_t *root)
{
    ...
    dm_get_shadow_data get_shadow_data_func = (type == DM_MSG_PROPERTY_SET) ? (dm_mgr_deprecated_get_property_data) : (dm_mgr_deprecated_get_service_input_data);
    dm_set_shadow_data set_shadow_data_func = (type == DM_MSG_PROPERTY_SET) ? (dm_mgr_deprecated_set_property_value) : (dm_mgr_deprecated_set_service_input_value);
